...
const entries = [
  {
    label: "NV3D 管线",
    icon: <NeonIcon name="Image" size={16}><Image className="h-4 w-4" /></NeonIcon>,
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-lg border border-white/10 bg-black/95 backdrop-blur-xl shadow-2xl z-[200] overflow-hidden max-h-[70vh] overflow-y-auto">
          <div className="px-3 py-2.5 border-b border-white/10 flex items-center gap-2 sticky top-0 bg-black/95 z-10">
            <NeonIcon name="Wrench" size={16}><Wrench className="h-3.5 w-3.5 text-cyan-400" /></NeonIcon>
              ...
              <div key={i} className="border-b border-white/5">
                ...
                >
                  ...
                  <span className="text-gray-500 mt-0.5">
                    {isOpen ? <NeonIcon name="ChevronDown" size={16}><ChevronDown className="h-4 w-4" /></NeonIcon> : <NeonIcon name="ChevronRight" size={16}><ChevronRight className="h-4 w-4" /></NeonIcon>}
                  </span>
                </button>
                ...
                {isOpen && e.children?.map((child, j) => (
                  <div key={j} className="pl-10 pr-3 pb-3 pt-1">
                    <div className="flex items-start gap-2">
                      <NeonIcon name="Terminal" size={16}><Terminal className="h-3.5 w-3.5 text-gray-600 mt-0.5 shrink-0" /></NeonIcon>
                      <div className="flex-1 min-w-0">
