...
import { Bug, FlaskConical, Wrench, Box, Image, Terminal, ChevronDown, ChevronRight } from "lucide-react";
...
interface DevEntry {
  ...
  phase: string;
  desc: string;
  children?: { label: string; desc: string; cmd: string }[];
}

const entries: DevEntry[] = [
  {
    label: "WebGL Benchmark",
    href: "/benchmark",
    icon: <FlaskConical className="h-4 w-4" />,
    phase: "阶段 0",
    desc: "性能基准测试 — 100K 三角面 + Bloom 压力场景",
  },
  {
    label: "集成验证",
    href: "/webgl3d-test",
  ...
  },
  {
    ...
    phase: "阶段 2-4",
    desc: "单文件 node pipeline.mjs — 环境校验 / 素材校验 / 生成 manifest / 打包 / 签名",
    children: [
      { label: "环境校验", desc: "Node.js / basisu / gltf-transform / draco_encoder", cmd: "node scripts/webgl3d/pipeline.mjs --source ../../nova-themes-assets/webgl3d/{theme} --steps env" },
      { label: "素材校验", desc: "命名/分辨率/manifest JSON/i18n/.glb 大小", cmd: "node scripts/webgl3d/pipeline.mjs --source ../../nova-themes-assets/webgl3d/{theme} --steps validate" },
      ...
      { label: "打包 .nv3d", desc: "manifest + 源文件 → 二进制 NV3D 文件", cmd: "node scripts/webgl3d/pipeline.mjs --source ../../nova-themes-assets/webgl3d/{theme} --steps pack" },
      { label: "Ed25519 签名", desc: "对 .nv3d 签名（需要 --sign-key）", cmd: "node scripts/webgl3d/pipeline.mjs --source ../../nova-themes-assets/webgl3d/{theme} --steps sign --sign-key <私钥路径>" },
      { label: "E2E 验证 (38 tests)", desc: "manifest→pack→逐字节验证→Rust 解析", cmd: "node scripts/webgl3d/e2e-test.mjs" },
...
export default function DevToolsMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-lg border border-white/10 bg-black/95 backdrop-blur-xl shadow-2xl z-[100] overflow-hidden max-h-[80vh] overflow-y-auto">
          {/* Header */}
          ...
          {entries.map((e, i) => {
            ...
            if (e.href) {
              return (
                <a key={i} href={e.href} onClick={() => setOpen(false)}
                  className="flex items-start gap-3 px-3 py-3 hover:bg-white/5 transition-colors border-b border-white/5">
                  <div className="shrink-0 mt-0.5 text-gray-400">{e.icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-200">{e.label}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-cyan-500/15 text-cyan-400 font-medium">{e.phase}</span>
                    </div>
            ...
            // Expandable entry (has children, no href)
            return (
              <div key={i} className="border-b border-white/5">
                ...
                <button
                  ...
                  onClick={(evt) => { evt.stopPropagation(); setExpanded(isOpen ? null : e.label); }}
                >
                  <div className="shrink-0 mt-0.5 text-gray-400">{e.icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
